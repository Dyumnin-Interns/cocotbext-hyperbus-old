...
class HyperbusClock():
    ...
    '''
      try:
        ...
        await self.transfer_ca_words(address, burst=False)  # Set burst=False for read
...
class AddressSpace(BusDriver):
    ...
    def translate_logical_address(self, address):
      if not (self.get_constraints()["min_address"] <= address <= self.get_constraints()["max_address"]):
